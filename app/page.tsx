...
function CostCalculator() {
  ...
  const [selectedModel, setSelectedModel] = useState<{ id: ModelId; provider: Provider }>({ id: "gemini-3.1-pro-preview", provider: "gemini" });
          ...
          <div>
            ...
            <select
              ...
              <optgroup label="xAI (Grok)">
                {ALL_MODELS.filter((m) => m.provider === "grok").map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
              </optgroup>
            </select>
            {guidance && (
            ...
            <p className="mt-1 text-[11px] text-zinc-400">Not sure? Gemini 3.1 Pro or Claude Sonnet 4.5 are great defaults.</p>
          </div>
        </div>
